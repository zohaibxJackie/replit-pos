...
export const api = {
  ...
  roles: {
    ...
    assign: (userId: string, roleId: string) =>
      request("/api/roles/assign", {
        method: "POST",
        body: { userId, roleId },
      }),

    remove: (userId: string, roleId: string) =>
      request("/api/roles/remove", {
        method: "POST",
        body: { userId, roleId },
      }),
  },
  ...
  loginHistory: {
    getByUser: (userId: string) => request(`/api/login-history/user/${userId}`),

    getAll: () => request("/api/login-history"),
  },

  userDetails: {
    get: (userId: string) => request(`/api/user-details/${userId}`),
  ...
  shops: {
    getAll: () => request("/api/shops"),

    getById: (id: string) => request(`/api/shops/${id}`),
    ...
    delete: (id: string) => request(`/api/shops/${id}`, { method: "DELETE" }),

    getStaff: (shopId: string) => request(`/api/shops/${shopId}/staff`),
  ...
  products: {
    getAll: (params?: {
      ...
      if (params?.lowStock !== undefined)
        searchParams.set("lowStock", params.lowStock.toString()); // ← ADD THIS
    ...
    getLowStock: (shopId?: string) =>
      request(
        shopId
          ? `/api/products/low-stock?shopId=${shopId}`
    ...
    bulkCreate: (data: {
      ...
      console.log(data);
      return request<{ products: Array<{ id: string }>; count: number }>(
        "/api/products/bulk",
    ...
    getVariants: (params?: { brandId?: string; categoryId?: string }) => {
      ...
      }>(query ? `/api/products/variants?${query}` : "/api/products/variants");
    },
  },
  ...
  vendors: {
    ...
    update: (
      ...
      }>(`/api/vendors/${id}`, { method: "PUT", body: data }),

    delete: (id: string) => request(`/api/vendors/${id}`, { method: "DELETE" }),
  },
  ...
  mobileCatalog: {
    getAll: (params?: {
      ...
      return request<{
        ...
        query
          ? `/api/products/catalog/mobiles?${query}`
          : "/api/products/catalog/mobiles"
      );
    },
    ...
    getModels: (brand: string) =>
      request<{
        ...
        `/api/products/catalog/mobiles/models?brand=${encodeURIComponent(
          brand
        )}`
      ),
    ...
    getColors: (selectedModel: object) => {
      ...
      return request<{ colors: Array<{ id: string; color: string }> }>(
        `/api/products/catalog/mobiles/colors?${params.toString()}`
      );
    },
    ...
    getItem: (
      ...
      }>(`/api/products/catalog/mobiles/item?${params.toString()}`);
    },
  },
  ...
  accessoryCatalog: {
    ...
    getAll: (params?: {
      ...
      return request<{
        ...
        query
          ? `/api/products/catalog/accessories?${query}`
  ...
  taxes: {
    getAll: (params?: { search?: string; isActive?: boolean }) => {
      ...
      }>(query ? `/api/taxes?${query}` : "/api/taxes");
    },
    ...
    create: (data: { name: string; value: number }) =>
      ...
      }>("/api/taxes", { method: "POST", body: data }),
    ...
    update: (
      ...
      }>(`/api/taxes/${id}`, { method: "PUT", body: data }),

    delete: (id: string) => request(`/api/taxes/${id}`, { method: "DELETE" }),
  },
  ...
  categories: {
    ...
    getById: (id: string) =>
      request<{ category: { id: string; name: string }; productCount: number }>(
        `/api/categories/${id}`
      ),

    create: (data: { name: string }) =>
      request<{ category: { id: string; name: string } }>("/api/categories", {
        method: "POST",
        body: data,
      }),

    update: (id: string, data: { name: string }) =>
      request<{ category: { id: string; name: string } }>(
        `/api/categories/${id}`,
  ...
  customers: {
    ...
    getById: (id: string) =>
      request<{ customer: CustomerType; recentSales: Array<any> }>(
        `/api/customers/${id}`
      ),

    create: (data: CustomerCreateType) =>
      request<{ customer: CustomerType }>("/api/customers", {
        method: "POST",
        body: data,
      }),

    update: (id: string, data: Partial<CustomerCreateType>) =>
      request<{ customer: CustomerType }>(`/api/customers/${id}`, {
        method: "PUT",
        body: data,
      }),
  ...
  variants: {
    create: (data: {
      ...
      return request<{ variant: { id: string; variantName: string } }>(
        "/api/variants",
        {
          method: "POST",
          body: JSON.stringify(data),
  ...
  activityLogs: {
    getAll: (params?: {
      ...
      return request<{
        ...
        query
          ? `/api/notifications/activity-logs?${query}`
          : "/api/notifications/activity-logs"
      );
    },
  },

  pricingPlans: {
    getAll: () => request("/api/pricing-plans"),

    getById: (id: string) => request(`/api/pricing-plans/${id}`),
  ...
  featureFlags: {
    getAll: () => request("/api/feature-flags"),

    getByName: (name: string) => request(`/api/feature-flags/${name}`),
  ...
  repairJobs: {
    getAll: (shopId?: string) =>
      request(
        shopId ? `/api/repair-jobs?shopId=${shopId}` : "/api/repair-jobs"
      ),

    getById: (id: string) => request(`/api/repair-jobs/${id}`),
  ...
  repairPersons: {
    getAll: (shopId?: string) =>
      request(
        shopId ? `/api/repair-persons?shopId=${shopId}` : "/api/repair-persons"
      ),
