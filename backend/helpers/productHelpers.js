...
import { stock, stockBatches, variant, product, brand, category, vendors } from '../../shared/schema.js';
import { eq, and, desc, sql, or, ne, inArray } from 'drizzle-orm';

export const fetchMobileProducts = async ({ userShopIds, queryShopId, search, lowStock, offset, pageLimit }) => {
...
export const fetchAccessoryProducts = async ({ userShopIds, queryShopId, search, lowStock, offset, pageLimit }) => {
  ...
  if (lowStock === 'true') {
    conditions.push(sql`${stockBatches.quantity} <= ${stockBatches.lowStockThreshold}`);
  }
