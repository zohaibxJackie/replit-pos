...
import { stock, variant, product, category, brand, vendors, shops, saleItems } from '../../shared/schema.js';
import { eq, sql, ilike, and, desc, or, inArray, ne } from 'drizzle-orm';
import { createStockSchema, updateStockSchema, bulkCreateStockSchema, stockFilterSchema } from '../validators/inventory.js';
...
export const getStock = async (req, res) => {
  try {
    ...
    const { page = 1, limit = 20, shopId: queryShopId, variantId, stockStatus, condition, vendorId, isSold, search } = validation.data;
...
export const createStock = async (req, res) => {
  try {
    ...
    const { 
      variantId, shopId, primaryImei, secondaryImei, serialNumber, barcode,
      purchasePrice, salePrice, stockStatus, notes, condition, lowStockThreshold, vendorId
    } = validation.data;
    ...
    const [var_] = await db.select().from(variant).where(eq(variant.id, variantId)).limit(1);
    if (!var_) {
    ...
    if (vendorId) {
      const [v] = await db.select().from(vendors).where(
...
export const bulkCreateStock = async (req, res) => {
  try {
    ...
    const { 
      variantId, shopId, purchasePrice, salePrice, condition, lowStockThreshold, vendorId, quantity, items
    } = validation.data;
    ...
    if (uniqueImeis.size !== allImeis.length) {
      return res.status(400).json({ error: req.t('stock.duplicate_imei_in_batch') });
    }

    for (const imei of allImeis) {
      if (!(await checkImeiUniqueness(imei))) {
        return res.status(409).json({ error: req.t('stock.imei_exists') + `: ${imei}` });
      }
    }
    ...
    try {
      await logActivity(req.user?.id, 'bulk_create', 'stock', null, {
        variantId,
        shopId,
        quantity,
...
export const updateStock = async (req, res) => {
  try {
    ...
    if (data.secondaryImei && data.secondaryImei !== existingStock.secondaryImei) {
    ...
    if (data.vendorId) {
      const [v] = await db.select().from(vendors).where(
        and(eq(vendors.id, data.vendorId), eq(vendors.shopId, existingStock.shopId))
      ).limit(1);
      if (!v) {
        return res.status(400).json({ error: req.t('stock.invalid_vendor') });
      }
    ...
    if (data.primaryImei !== undefined) updateData.primaryImei = data.primaryImei;
    if (data.secondaryImei !== undefined) updateData.secondaryImei = data.secondaryImei;
    if (data.serialNumber !== undefined) updateData.serialNumber = data.serialNumber;
    if (data.barcode !== undefined) updateData.barcode = data.barcode;
    if (data.purchasePrice !== undefined) updateData.purchasePrice = data.purchasePrice?.toString() || null;
