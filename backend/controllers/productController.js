...
export const getProducts = async (req, res) => {
  try {
    ...
    // Use `req.userShopIds` set by middleware
...
export const createProduct = async (req, res) => {
  try {
    ...
    if (!shopId) {
      return res.status(400).json({
        error: req.t ? req.t("product.shop_required") : "Shop ID is required",
      });
    }
    ...
    if (!variantId) {
      return res.status(400).json({
        error: req.t
          ? req.t("product.variant_required")
    ...
    if (!vendorType) {
      return res.status(400).json({
        error: req.t ? req.t("product.vendor_type") : "Vendor type is required",
      });
    } else if (vendorType === process.env.WHOLESALER) {
      const wholesaler = await db
        .select()
        .from(users)
        .where(and(eq(users.role, vendorType), eq(users.id, vendorId)))
        .limit(1);
      if (!wholesaler) {
        return res.status(400).json({
          error: req.t ? req.t("product.invalid_vendor") : "Invalid vendor",
        });
      }
      ...
      const customer = await db
        .select()
        .from(customers)
        .where(eq(customers.id, vendorId))
        .limit(1);
      if (!customer) {
        return res.status(400).json({
          error: req.t ? req.t("product.invalid_vendor") : "Invalid vendor",
    ...
    if (primaryImei) {
      if (!(await checkImeiUniqueness(primaryImei))) {
        return res.status(409).json({
          error: req.t ? req.t("product.imei_exists") : "IMEI already exists",
        });
      }
      ...
      if (primaryImei && secondaryImei && primaryImei === secondaryImei) {
        return res.status(400).json({
          error: req.t
            ? req.t("product.imei_duplicate")
            : "Primary and Secondary IMEI cannot be the same",
        });
      }
    }
    ...
    if (taxId) {
      ...
      if (!tax) {
        return res.status(400).json({
          error: req.t ? req.t("product.tax_invalid") : "Invalid tax id",
        });
      }
    }

    const finalBarcode =
      barcode || `STK-${shopId.substring(0, 6)}-${Date.now()}`;
...
export const bulkCreateProducts = async (req, res) => {
  try {
    ...
    if (quantity !== imeis.length) {
      return res.status(400).json({
        error: req.t
          ? req.t("product.imei_count_mismatch")
          : "IMEI count must match quantity",
      });
    }
    ...
    if (uniqueImeis.size !== allImeis.length) {
      return res.status(400).json({
        error: req.t
          ? req.t("product.duplicate_imei_in_batch")
          : "Duplicate IMEIs found in batch",
      });
    }
    ...
    res.status(201).json({
      ...
      message: req.t
        ? req.t("product.bulk_created", { count: quantity })
        : `${quantity} stock items created successfully`,
    });
  } catch (error) {
    console.error("Bulk create products error:", error);
    res.status(500).json({
      error: req.t
        ? req.t("product.bulk_create_failed")
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (primaryImei !== undefined) updateData.primaryImei = primaryImei || null;
    ...
    if (salePrice !== undefined)
      updateData.salePrice = salePrice != null ? salePrice.toString() : null;
    if (notes !== undefined) updateData.notes = notes;
...
export const getMobileCatalog = async (req, res) => {
  try {
    ...
    const [mobileCategory] = await db
      .select()
      .from(categories)
      .where(ilike(categories.name, "mobile%"))
      .limit(1);
    if (mobileCategory) {
...
export const getMobileCatalogColors = async (req, res) => {
  try {
    ...
    let conditions = [eq(product.id, productId)];
...
export const getMobileCatalogItem = async (req, res) => {
  try {
    ...
    if (!brandName || !model) {
      return res
        .status(400)
        .json({ error: req.t("product.brand_model_required") });
    }
    ...
    if (!catalogItem) {
      return res
        .status(404)
        .json({ error: req.t("product.catalog_item_not_found") });
    }
