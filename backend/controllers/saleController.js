...
export const getSaleById = async (req, res) => {
  try {
    ...
    const [sale] = await db
      .select()
      .from(sales)
      .where(and(eq(sales.id, id), eq(sales.shopId, req.userShopIds?.[0])))
      .limit(1);
...
export const createSale = async (req, res) => {
  try {
    const { customerId, paymentMethod, items, discount, tax } =
      req.validatedBody;
    const shopId = req.userShopIds?.[0];
...
export const getTodaySales = async (req, res) => {
  try {
    ...
    const todaySales = await db
      .select()
      .from(sales)
      .where(and(eq(sales.shopId, shopId), gte(sales.createdAt, today)))
      .orderBy(desc(sales.createdAt));

    const [{ totalAmount, saleCount }] = await db
      .select({
        totalAmount: sql`COALESCE(SUM(total), 0)::decimal`,
...
export const getSalesAnalytics = async (req, res) => {
  try {
    ...
    const [stats] = await db
      ...
      .from(sales)
      .where(and(eq(sales.shopId, shopId), gte(sales.createdAt, startDate)));
