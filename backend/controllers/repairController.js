...
import { repairJobs, repairPayments, repairPersons, customers } from '../../shared/schema.js';
...
export const getRepairJobById = async (req, res) => {
  try {
    ...
    if (job.repairPersonId) {
      const [person] = await db.select().from(repairPersons).where(eq(repairPersons.id, job.repairPersonId)).limit(1);
      repairPerson = person;
    }
...
export const createRepairJob = async (req, res) => {
  try {
    const {
      customerId, customerName, customerPhone, customerDni,
      deviceBrand, deviceModel, imei, defectSummary, problemDescription,
    ...
    if (autoAssign && !repairPersonId) {
      const [availablePerson] = await db.select()
        .from(repairPersons)
        .where(and(eq(repairPersons.shopId, shopId), eq(repairPersons.isAvailable, true)))
        .limit(1);
      ...
      const [person] = await db.select().from(repairPersons).where(eq(repairPersons.id, repairPersonId)).limit(1);
      if (person) {
        repairPersonName = person.name;
      }
    }

    const [newJob] = await db.insert(repairJobs).values({
      shopId,
      ticketNumber,
...
export const updateRepairJob = async (req, res) => {
  try {
    ...
    const [updatedJob] = await db.update(repairJobs)
      .set(updateData)
...
export const updateRepairPerson = async (req, res) => {
  try {
    ...
    const [updatedPerson] = await db.update(repairPersons)
      .set(updateData)
